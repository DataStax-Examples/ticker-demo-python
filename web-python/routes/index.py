...
def preflight_check():
    ...
    prepared_statements['get_recommendations'] = cassandra_helper.session.prepare('''
...
@index_api.route('/disclaimer')
def disclaimer():
    return render_template('/disclaimer.jinja2')


@index_api.route('/dash')
def dash():
    return render_template('/dash.jinja2')


@index_api.route('/search')
def search():
    preflight_check()
    ...
    try:
        ...
        if len(search_results.current_rows) == 0:
            alert = {
                'level': 'info',
...
@index_api.route('/portfolio', methods=['GET', 'POST'])
def portfolio():
    preflight_check()
...
@index_api.route('/recommendations', methods=['GET', 'POST'])
def recommendations():
    preflight_check()
...
@index_api.route('/buy', methods=['POST'])
def buy():
    preflight_check()
    values = {
        'email_address': session['email_address'],
